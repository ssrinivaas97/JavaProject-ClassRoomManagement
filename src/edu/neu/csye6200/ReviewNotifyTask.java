	...
	public static void teacherReg() {
		...
		for(Teacher s :GroupHelper.teachers) {
			...
			if( tDay.getTime().compareTo(curDate.getTime()) == 0 ) {
				...
				/*
				 * Mailer m = new Mailer(s.getEmailID()); m.setRecpName(s.getFirstName() + " " +
				 * s.getLastName()); m.setStudName(s.getFirstName() + " " + s.getLastName());
	...
	public static void studentReg() {
		...
		for(Student s :GroupHelper.students) {
			...
			if( tDay.getTime().compareTo(curDate.getTime()) == 0 ) {
				 ...
				 * m.setRecpName(s.getGuardianName()); m.setStudName(s.getFirstName() + " " +
				 * s.getLastName());
				 * m.setRevDate(ConversionHelper.DateToString(s.getAnnualRegistrationDate()));
				 * m.setPhoneNum(s.getPhoneNumber()); m.sendAnnualRegistrationMessage();
				 */
			}
