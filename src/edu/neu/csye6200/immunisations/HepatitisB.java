...
public class HepatitisB extends AbstractImmunization{
	...
	public void checkImmunization(Student s) {
		System.out.println("Inside checkImmunization from  HepatitisB");
		/*
		 * Mailer m = new Mailer(s.getGuardianEmail()); if(immuDate.size() < 3 &&
		 * s.getAge() >= 15) { System.out.println(s.getAge());
		 ...
		 * s.getAge() >= 18) { m.setVaccine(this.immunizationName); m.setDose("3");
		 * m.setStudName(s.getFirstName() + " " + s.getLastName());
		 * m.setRecpName(s.getGuardianName()); m.setPhoneNum(s.getPhoneNumber());
