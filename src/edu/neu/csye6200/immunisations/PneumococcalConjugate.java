...
public class PneumococcalConjugate extends AbstractImmunization {
	...
	public void checkImmunization(Student s) {
		...
		System.out.println("Inside checkImmunzation in PneumococcalConjugate");
		/*
		 * Mailer m = new Mailer( s.getGuardianEmail()); if(immuDate.size() < 3 &&
		 * this.age >= 6 && this.age <= 8) { m.setVaccine(this.immunizationName);
		 * m.setDose("3"); m.setStudName(s.getFirstName() + " " + s.getLastName());
		 * m.setRecpName(s.getGuardianName()); m.setPhoneNum(s.getPhoneNumber());
		 * m.sendImmunizationNotificationMail(); } else if(immuDate.size() < 3 &&
		 * this.age > 8) { m.setVaccine(this.immunizationName); m.setDose("3");
		 * m.setStudName(s.getFirstName() + " " + s.getLastName());
		 * m.setPhoneNum(s.getPhoneNumber()); m.setRecpName(s.getGuardianName());
		 * m.sendImmunizationWarning(); } else if(immuDate.size() == 3 && this.age >=12
		 * && this.age <=18) { m.setVaccine(this.immunizationName); m.setDose("4");
		 * m.setStudName(s.getFirstName() + " " + s.getLastName());
		 * m.setRecpName(s.getGuardianName()); m.setPhoneNum(s.getPhoneNumber());
		 * m.sendImmunizationNotificationMail(); } else if(immuDate.size() == 3 &&
		 * this.age >=18) { m.setVaccine(this.immunizationName); m.setDose("3");
		 * m.setStudName(s.getFirstName() + " " +s.getLastName());
		 * m.setRecpName(s.getGuardianName()); m.setPhoneNum(s.getPhoneNumber());
		 * m.sendImmunizationWarning(); } else if(immuDate.size() < 3 && this.age >= 12)
		 * { m.setVaccine(this.immunizationName); m.setDose("3 and 4");
		 * m.setStudName(s.getFirstName() + " " + s.getLastName());
		 * m.setRecpName(s.getGuardianName()); m.setPhoneNum(s.getPhoneNumber());
